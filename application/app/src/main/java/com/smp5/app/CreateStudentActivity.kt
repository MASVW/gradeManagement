...
class CreateStudentActivity : ComponentActivity() {
    ...
    private fun setupListener(){
        buttonCreate.setOnClickListener{
            ...
            api.createStudent(name, age, studentClassStr).enqueue(object : Callback<SubmitModel> {
                override fun onResponse(
                    call: Call<SubmitModel>,
                    response: Response<SubmitModel>
                ) {
                    if (response.isSuccessful) {
                        Toast.makeText(this@CreateStudentActivity, "Siswa berhasil dibuat", Toast.LENGTH_SHORT).show()
                        finish()
                    } else {
                        Toast.makeText(this@CreateStudentActivity, "Gagal membuat siswa", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onFailure(call: Call<SubmitModel>, t: Throwable) {
                    Log.e(TAG, "API call failed: ${t.message}")
                    Toast.makeText(this@CreateStudentActivity, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
                }
            })
        }
    }
